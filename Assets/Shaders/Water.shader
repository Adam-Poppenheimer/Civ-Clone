...
CGINCLUDE
	...
	half4 frag( v2f i ) : SV_Target
	{
		...
		half4 baseColor = ExtinctColor (i.color, i.viewInterpolator.w * _InvFadeParemeter.w);
		#ifdef WATER_REFLECTIVE
	...
	v2f_simple vert200(appdata_full v)
	{
		...
		float4 waterFlow = v.texcoord2 * _Time.x;

		half3 worldSpaceVertex = mul(unity_ObjectToWorld, v.vertex).xyz;
		half2 tileableUv = worldSpaceVertex.xz + waterFlow.xz;
